      ...
      <div class="content">
         <div class="container-fluid">
          ...
          Connection conn1= (Connection)DriverManager.getConnection(url,user,passwd);
            	 ...
            	 ResultSet rs2=stmt1.executeQuery("select * from enseignant  where idformation="+idformation);
           while(rs2.next()){
             %>
		    <tr>
			    <td> <%=rs2.getInt("idenseignant") %> </td>
				<td> <%=rs2.getString("nom") %> </td>
			    <td> <%=rs2.getString("prénom") %> </td>
			    
			    
			    <td><a class="nav-link" href="http://localhost:8080/gestion/SENSemploi?idens=<%= rs2.getInt("idenseignant") %>&idformation=<%=idformation %>&iduser=${sessionScope.iduser}&name=${sessionScope.name}"  class="btn btn-round btn-fill btn-info">
                                <i class=""><img src="../assets/img/cal.PNG" ></i> </a>
